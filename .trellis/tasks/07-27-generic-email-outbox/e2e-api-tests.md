...
| --- | --- | --- | --- | --- | --- | --- |
| E2E-001 | 创建 active managed user | 管理权限与 SMTP stub | `POST /api/v1/users/` 合法 payload | 现有创建响应 | 同事务创建 ACCOUNT_SET_PASSWORD outbox；HTTP 不调用 SMTP | 禁用用户不创建 invitation；outbox/模板不含明文 password，User 密码哈希仍保持 API 兼容 |
