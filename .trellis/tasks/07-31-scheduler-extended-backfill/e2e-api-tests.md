...
- Isolation: run against a dedicated `aiadmin_d003_test` PostgreSQL database
  created/migrated for this task; do not write fixtures to the development DB.
...
| --- | --- | --- | --- | --- | --- | --- |
| E2E-001 | `POST /api/v1/scheduler/jobs/{job_id}/backfill` | Enabled job using D-001 allowed class, Cron `0 8 * * *`, no active run; actor has `scheduler.jobs.manage` | `{"planned_at":"<now_minus_365_days_plus_one_minute_in_shanghai>+08:00"}` where the time is an exact Cron occurrence | `200`; body has `trigger=MANUAL_BACKFILL`, `status=QUEUED`, UTC `planned_at`, and actor `requested_by` | Exactly one new run; class path/config equal the job snapshot; `next_dispatch_at` is set; no direct Celery call | Assert the exact 365-day boundary separately with a deterministic service clock; this API case avoids current-second/minute precision drift |
...
2. Apply test fixtures and run E2E-001 through E2E-008 against the local API.
