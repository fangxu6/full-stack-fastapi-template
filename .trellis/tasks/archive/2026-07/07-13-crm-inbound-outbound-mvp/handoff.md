...
- 已关联后续修正任务：`07-14-inventory-decimal-rolls`。该任务将匹数改为最多两位小数，并修复历史导入时 `0.5` 匹被截断为 `0` 导致成品出货列表 500 的问题。
- 任务状态：`in_progress`。
...
- 续作验证：库存 API 扩展为 7 项，连同 6 项导入器测试在隔离数据库全部通过；完整后端 lint 和前端生产构建通过。隔离服务的健康检查与前端页面均可启动，但当前运行时没有可用浏览器连接，尚不能完成浏览器 E2E。

## Known Blockers

### Frontend Router Dependency

此前的 Router 依赖问题未在当前环境复现；`bun run build` 已成功完成。E2E 的唯一现状阻塞是当前运行时无可用浏览器连接；恢复后按四组页面和单据全流程执行。

### Historical Workbook Input

两份 `hongxia` Excel 文件位于工作区，且已在隔离数据库完成实际导入验证。主本地数据库的 Alembic 历史已修复并升级至 `c9b1f4e7a2d0`；首次实际导入前仍应保留 dry-run 对账报告。
