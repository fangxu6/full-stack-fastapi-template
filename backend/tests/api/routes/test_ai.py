...
def test_enabled_inventory_bff_audits_an_unavailable_orchestrator(
    client: TestClient,
    db: Session,
    superuser_token_headers: dict[str, str],
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        settings,
        "AI_ACTOR_GRANT_SIGNING_KEY",
        "test-grant-signing-key-at-least-32-bytes",
    )
    response = client.post(
        "/api/v1/ai/inventory/query",
        headers=superuser_token_headers,
    ...
    assert run.status is AiRunStatus.FAILED
    assert run.error_category == "orchestrator_unavailable"


def test_enabled_inventory_bff_returns_the_completed_sidecar_response(
    client: TestClient,
    db: Session,
    superuser_token_headers: dict[str, str],
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        "app.modules.ai.service.call_inventory_sidecar",
        lambda **_: AiSidecarCompletedResponse(
            status="completed",
            answer="当前无成品库存余额。",
            citations=[
                {
...
def test_internal_balances_requires_credentials_then_uses_inventory_projection(
    ...
    tool_call = db.exec(select(AiToolCall).where(AiToolCall.run_id == run.id)).one()
    assert tool_call.status is AiToolCallStatus.COMPLETED
