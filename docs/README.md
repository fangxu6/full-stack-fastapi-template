...
- `docs/specs/trellis-codex-hooks-subagents/`：Trellis/Codex hooks 与 subagent 的当前契约和验证记录，说明默认 `auto`、`inline` 退出模式、兼容的 `sub-agent` 别名，以及 `SubagentStart`/`SubagentStop` 边界。
...
- `docs/rules/AI编码工作流.md`：把 AI coding 调研结论落到本仓库的日常开发流程，强调短 leash、当前源码核验、Trellis、规范上下文和验证门禁。
