...
2. **先读真实上下文。** 代码事实优先于记忆和模板。这个仓库有 `.codegraph/`，理解调用链、边界和影响面时先用 CodeGraph，再用 `rg` 补充。
...
2. **定位事实。** 用 CodeGraph / `rg` 找当前入口、调用链、测试和相邻模式。
...
```text
目标：修复/实现 <具体目标>。
上下文：请先读 AGENTS.md、相关 .trellis/spec，并用 CodeGraph/rg 找真实调用链。
